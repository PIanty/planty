...
export const getAndDeployContracts = async () => {
    ...
    await x2EarnApps.addApp(admin.address, admin.address, 'JusCat');
    const APP_ID = await x2EarnApps.hashAppName('JusCat');
    ...
    await x2EarnApps.connect(admin).addRewardDistributor(APP_ID, jusCatAddress);

    return {
        token: rewardToken,
        jusCat: jusCatInstance,
        x2EarnApps,
        x2EarnRewardsPool,
        appId: APP_ID,
        owner,
