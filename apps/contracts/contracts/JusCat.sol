...
import './interfaces/IJusCatPassport.sol';
import '@openzeppelin/contracts/utils/Strings.sol';

/**
 * @title JusCat Contract
 ...
 * Participants now require a JusCatPassport to make submissions.
 *
 ...
 * This contract can be initially deployed without this information and DEFAULT_ADMIN_ROLE can update it later through {JusCat-setAppId}.
 *
 ...
 */
contract JusCat is AccessControl {
    // The X2EarnRewardsPool contract used to distribute rewards
    IX2EarnRewardsPool public x2EarnRewardsPoolContract;

    // JusCatPassport contract for validation
    IJusCatPassport public jusCatPassport;

    // Flag to enable/disable passport requirement
    bool public passportRequired = false;
    ...
    event PassportRequirementUpdated(bool required);
    event JusCatPassportUpdated(address newPassportContract);

    /**
     * @dev Constructor for the JusCat contract
    ...
    constructor(address _admin, address _x2EarnRewardsPoolContract, uint256 _cycleDuration, uint256 _maxSubmissionsPerCycle, bytes32 _appId) {
        require(_admin != address(0), 'JusCat: _admin address cannot be the zero address');
        require(_x2EarnRewardsPoolContract != address(0), 'JusCat: x2EarnRewardsPool contract address cannot be the zero address');
    ...
    function registerValidSubmission(address participant, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(amount > 0, 'JusCat: Amount must be greater than 0');
        require(submissions[getCurrentCycle()][participant] < maxSubmissionsPerCycle, 'JusCat: Max submissions per user reached');
        require(rewardsLeft[getCurrentCycle()] >= amount, 'JusCat: Not enough rewards left');
        require(block.number < getNextCycleBlock(), 'JusCat: Cycle is over');
        
        // Check if participant has a passport if required
        if (passportRequired) {
            require(address(jusCatPassport) != address(0), 'JusCat: Passport contract not set');
            require(jusCatPassport.hasPassport(participant), 'JusCat: Participant does not have a passport');
        }
    ...
    function setRewardsAmount(uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(amount <= x2EarnRewardsPoolContract.availableFunds(appId), 'JusCat: Insufficient balance on the X2EarnRewardsPool contract');
    ...
    function withdrawRewards(uint256 cycle) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(rewards[cycle] > 0, 'JusCat: No rewards to withdraw');
        require(cycle < getCurrentCycle(), 'JusCat: Cycle is not over');
    ...
    function setMaxSubmissionsPerCycle(uint256 _maxSubmissionsPerCycle) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_maxSubmissionsPerCycle > 0, 'JusCat: Max submissions per cycle must be greater than 0');
        maxSubmissionsPerCycle = _maxSubmissionsPerCycle;
    }
    ...
    /**
     * @dev Sets the JusCatPassport contract address
     * @param _passportContract The address of the JusCatPassport contract
     */
    function setJusCatPassport(address _passportContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_passportContract != address(0), 'JusCat: Passport contract address cannot be the zero address');
        jusCatPassport = IJusCatPassport(_passportContract);
        emit JusCatPassportUpdated(_passportContract);
    }

    /**
     * @dev Enables or disables the passport requirement
     * @param _required Whether the passport is required
     */
    function setPassportRequired(bool _required) external onlyRole(DEFAULT_ADMIN_ROLE) {
        passportRequired = _required;
        emit PassportRequirementUpdated(_required);
    }
    ...
    /**
     * @dev Checks if a participant has a passport
     * @param participant Address of the participant
     * @return True if the participant has a passport, otherwise false
     */
    function hasUserPassport(address participant) public view returns (bool) {
        if (address(jusCatPassport) == address(0)) {
            return false;
        }
        return jusCatPassport.hasPassport(participant);
    }
}
