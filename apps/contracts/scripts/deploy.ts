...
export async function deploy() {
    ...
    if (network.name === 'vechain_solo') {
        ...
        await x2EarnApps.addApp(deployer.address, deployer.address, 'JusCat');
        const appID = await x2EarnApps.hashAppName('JusCat');
        APP_ID = appID;
    ...
    console.log('Deploying JusCat contract...');
    const jusCat = await ethers.getContractFactory('JusCat');

    const jusCatInstance = await jusCat.deploy(
        deployer.address,
    ...
    await jusCatInstance.waitForDeployment();

    const jusCatAddress = await jusCatInstance.getAddress();
    console.log(`JusCat deployed to: ${jusCatAddress}`);

    console.log('To start using the contract, we need to set the rewards amount and switch to the next cycle');

    const rewardsAmountResult = await (await jusCatInstance.setRewardsAmount(1000000000000000000000n)).wait();
    ...
    const nextCycleResult = await (await jusCatInstance.setNextCycle(2n)).wait();
    ...
    // In solo network, we need to add the JusCat contract as a distributor
    if (network.name === 'vechain_solo') {
        console.log('Add JusCat contracts as distributor...');
        const x2EarnApps = await ethers.getContractAt('X2EarnAppsMock', X2EARN_APPS);
        await x2EarnApps.addRewardDistributor(APP_ID, jusCatAddress);
        console.log('Added');
    }
    ...
    updateConfig(
        {
            ...config,
            CONTRACT_ADDRESS: jusCatAddress,
            TOKEN_ADDRESS: REWARD_TOKEN_ADDRESS,
        },
        jusCatAbi,
        null // No passport ABI in main deployment
    );
